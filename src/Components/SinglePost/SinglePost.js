...
const SinglePost = ({ post }) => {
    ...
    const handelLike = likePost => {
        console.log('like', likePost)
        let likedata = parseInt(post.like) + 1;
        setLike(likedata)
        
        console.log(likedata)
        
        fetch(`http://localhost:5000/post/${likePost._id}`, {
            method: 'PUT',
            headers: {
        ...
        <div className=' m-3'>
            <div className="flex flex-col max-w-lg p-6 space-y-6 overflow-hidden rounded-lg shadow-md dark:bg-gray-900 dark:text-gray-100">
                <div className="flex space-x-4">
                    <img alt="" src={post.imageurl} className="object-cover w-12 h-12 rounded-full shadow dark:bg-gray-500" />
                    <div className="flex flex-col space-y-1">
                        <Link to="#" className="text-sm font-semibold">{post.name}</Link>
                        <span className="text-xs dark:text-gray-400">4 hours ago</span>
                    </div>
                </div>
                <div>
                    <img src={post.imageurl} alt="" className="object-cover w-full mb-4 h-60 sm:h-96 dark:bg-gray-500" />
                ...
                <div className="flex flex-wrap justify-between">
                    ...
                    <div className="flex space-x-2 text-sm dark:text-gray-400">
                        ...
                        <button onClick={()=> handelLike(post)} type="button" className="flex items-center p-1 space-x-1.5">
